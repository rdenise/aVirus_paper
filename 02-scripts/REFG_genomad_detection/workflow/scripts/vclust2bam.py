...
def process_alignments(aln_file, ani_file, output_bam, min_ani, fasta_gz):
    """
    ...
    ).sort(['query', 'qcov', 'ani', 'alnlen'], descending=[False, True, True, True])
    )
...
def main():
    args = parse_args()
    process_alignments(args.input, args.ani, args.output, args.min_ani, snakemake.input.fasta)
