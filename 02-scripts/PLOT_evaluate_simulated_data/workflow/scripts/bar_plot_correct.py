...
def count_rank_assignation(taxo_path, profiler, big_unwanted_list):
    ...
    for profiler_rank, ictv_rank in rank:
        ...
        full_count_melt.append(count_df)
    
    print("Full list with full_count_melt")

    full_count_melt = pl.concat(full_count_melt, how='align').sort(['Deamination','Coverage_sort']).drop(['Coverage_sort'])

    print("Finished with full_count_melt")
...
def plot_true_false(df):
    ...
    invert_taxonomy_order = {value:key for key, value in taxonomy_order.items()}

    df = df.with_columns(
        pl.col("Rank").str.head(3).replace_strict(taxonomy_order),
    ...
    df = df.with_columns(
        ...
        pl.col("Rank").replace_strict(invert_taxonomy_order),
    )
    
    p = (
        so.Plot(df, x="Rank", y="Percentage", color="x == y")
        .layout(size=(12, 9)) # width, height
        .facet("Coverage", "Deamination",  order={"row": ["None", "Light", "Heavy"], "col": ["0.1x", "0.5x", "1x", "3x", "5x", "10x"]})
        .add(so.Bar(), so.Stack())
...
count_melt = count_rank_assignation(
