...
# def calculate_sensitivity(column):
#     nb_reads = column.shape[0]
...
def create_dataframe_mean(list_of_files, big_unwanted_list, reads2size):
...
def plot_sensitivity_length(df):
    ...
    p = (
        so.Plot(df, x="Read Length", y="Sensitivity", color="damage_software", linestyle="Level")
...
# p = plot_sensitivity_length(df_mean[df_mean['Read Length'].isin(range(35, 501, 10))])
p = plot_sensitivity_length(df_mean.filter(pl.col('Read Length').is_in(range(35, 200, 5))))
